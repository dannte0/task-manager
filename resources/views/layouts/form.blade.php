...
<div class="flex justify-center items-center my-[50px]">
    <form class="rounded-xl shadow-lg bg-blue-200 flex flex-col gap-2 w-[750px] p-5" action="{{ isset($task) ? route('tasks.update', $task) : route('tasks.store') }}" method="POST">
        ...
        @error('title')
            <p>{{ $message }}</p>
        @enderror
        <input type="text" name="title" class="outline outline-gray-400 rounded transition delay-100 bg-zinc-50  focus:outline-gray-600 p-2 text-xl" id="" value="{{ $task->title ?? old('title') }}">
        <label for="Details" class="text-2xl">Details:</label>
        @error('title')
            <p>{{ $message }}</p>
        @enderror
        <textarea name="details" class="outline outline-gray-400 rounded transition delay-100 bg-zinc-50  focus:outline-gray-600 p-2 text-xl resize-none" id="" rows="8">{{ $task->details ?? old('details') }}</textarea>
        <div class="mt-3">
